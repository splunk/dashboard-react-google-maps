...
const convertRgbToHex_1 = require("./convertRgbToHex");
const validInput = [
    [0, 0, 0],
    [255, 255, 255],
    [255, 0, 0],
...
describe('convertRgbTohex', () => {
    it('should return correct hex for Rgb', () => {
        validInput.map((input, key) => {
            expect(convertRgbToHex_1.convertRgbToHex(input[0], input[1], input[2])).toBe(validOutput[key]);
        });
    });
    it('should throw an error when input is not valid', () => {
        invalidInput.map(input => {
            try {
                convertRgbToHex_1.convertRgbToHex(input[0], input[1], input[2]);
            }
            catch (e) {
