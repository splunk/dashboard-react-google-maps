...
const convertHexToRgb_1 = require("./convertHexToRgb");
const validInput = [
    '000000',
    'ffFFff',
    'ff0000',
...
describe('convertHexToRgb', () => {
    it('should return correct rgb for hex', () => {
        validInput.map((input, key) => {
            expect(convertHexToRgb_1.convertHexToRgb(input)).toStrictEqual(validOutput[key]);
        });
    });
    it('should throw an error when input is not valid', () => {
        invalidInput.map(input => {
            try {
                convertHexToRgb_1.convertHexToRgb(input);
            }
            catch (e) {
